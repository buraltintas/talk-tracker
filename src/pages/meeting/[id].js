...
const Meeting = () => {
                    ...
                    meeting?.participants.map((participant, idx) => {
                        return (
                            <div className={styles.participant}>
                                ...
                                <span data-cy='toggle-button' onClick={() => toggleParticipantTalked(idx)} className={styles.talkedIndicator}>{participant.isTalked ? '👍': '-'}</span>
                            </div>
                        )
                    })
                }
    ...
    const render = () => {
        if (!meeting) {
            return (
                <span>Cannot find this meeting.</span>
            )
        } else {
            return (
                <>
                    <span data-cy='meeting-name' className={styles.meetingName}>{meeting?.meetingName}</span>
                    <span className={styles.isTalked}>Is Talked?</span>
                    {renderParticipants()}
                    <span onClick={clearAllIsTalkedValues} className={styles.clear}>Clear</span>
                    <div className={styles.goHomeContainer}>
                        <Link data-cy='go-home-button' className={styles.ctaButton} href={'/'}>Go Home</Link>
                    </div>
                </>
            )
        }
