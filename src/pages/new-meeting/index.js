...
const NewMeeting = () => {
    ...
    const deleteParticipant = (index) => {
        const newParticipants = newMeeting.participants.filter((item, idx) => idx !== index);
    ...
    const renderParticipants = () => {
        return (
            newMeeting.participants.map( (person, idx) => {
                return (
                    <div className={styles.participants} key={`${person.name + idx}`}>
                        <span className={styles.index}>{idx + 1}</span>
                        <span className={styles.name}>{person.name}</span>
                        <span onClick={() => deleteParticipant(idx)} className={styles.delete}>❌</span>
                    </div>
                )
            } )
        )
    ...
    return (
        <div className={styles.newMeetingContainer}>
            ...
            <div className={styles.newMeetingForm}>
                ...
                />
                <NewParticipant addNewParticipant={(person => addNewParticipant(person))} />
                {renderParticipants()}
            </div>
            {newMeeting.participants.length > 1 && 
                <div className={styles.saveButtonContainer}>
                    <button data-cy='save-meeting-button' className={styles.button} onClick={saveMeeting}>Save Meeting</button>
                </div>
            }
        </div>
    )
